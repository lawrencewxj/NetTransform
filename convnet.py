...
class ConvNet(nn.Module):
    def __init__(self, net_dataset):
        ...
        self.conv1 = nn.Conv2d(out_channels=8,
    ...
    def netmorph_wider(self, widening_factor):
        ...
        self.conv1, self.conv2, self.bn1 = wider(
            self.conv1, self.conv2, self.conv1.out_channels * widening_factor, self.bn1)
        self.conv2, self.conv3, self.bn2 = wider(
            self.conv2, self.conv3, self.conv2.out_channels * widening_factor, self.bn2)
        self.conv3, self.fc1, self.bn3 = wider(
            self.conv3, self.fc1, self.conv3.out_channels * widening_factor, self.bn3)

        # TODO: check for FC layers
        # _, self.fc2, _ = wider(self.fc1, self.fc2, self.fc1.out_channel * widening_factor, None)
