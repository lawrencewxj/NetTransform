...
def wider(m1, m2, new_width, bnorm=None, out_size=None, noise=True,
    ...
    if "Conv" in m1.__class__.__name__ or "Linear" in m1.__class__.__name__:
        ...
        if nw1.dim() == 4:
            # in cuda device, new values as initiated as 0 in server, whereas in cpu in my local machine,
            # random values are assigned when the last dimension of tensor is increased,
            # it flattens the data and copies into new tensor and remaining is set to zero
